...
(async () => {
	...
	await page.addScriptTag({content: fs.readFileSync("timemachine.js", "utf8").replace("%timestamp%", config.timestamp)});
	await page.addScriptTag({content: `window.supremetohru = "${config.tohru}"`});
